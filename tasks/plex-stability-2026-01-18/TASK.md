...
- [x] Read project `AGENTS.md`: Quoted secret management rules from `projects/flippanet/AGENTS.md` in progress.md
- [x] Read `.ralph/docs/RALPH_RULES.md`: Quoted "Golden Rule" from TL;DR section in progress.md
- [x] Query Local RAG for "plex stability optimization transcoding": Documented findings in progress.md
...
- [ ] Verify SSH connectivity: `ssh -i ~/.ssh/flippanet flippadip@flippanet "echo 'connected'"`
- [ ] Verify Plex running: `ssh -i ~/.ssh/flippanet flippadip@flippanet "docker ps | grep plex"`
- [ ] Verify Preferences.xml path exists: `ssh -i ~/.ssh/flippanet flippadip@flippanet "ls -la '/var/lib/docker/volumes/flippanet_plex-config/_data/Library/Application Support/Plex Media Server/Preferences.xml'"`
...
- **Plex Container**: Uses `network_mode: host` so Plex binds directly to host network (no port mapping needed)
...
- **Tautulli**: Essential for monitoring - check regularly for buffer warnings and transcode failures
